...
func setupWorktreeTestRepo(t *testing.T) (string, string) {
	// Use the existing setupTestRepo helper to create and configure the main repo
...
func TestGitRepo_WorktreeCreateStash(t *testing.T) {
	...
	if len(hash) != gitSHALength {
		t.Errorf("CreateStash() hash length = %d, want %d", len(hash), gitSHALength)
	}
...
func TestGitRepo_WorktreeGetDiff(t *testing.T) {
	...
	hash := string(output)[:gitSHALength] // Get full SHA
...
func TestGitRepo_WorktreeObjectExists(t *testing.T) {
	...
	commitHash := string(output)[:gitSHALength]
